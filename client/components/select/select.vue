<template>
  <div
    :class="{ 'c-select_active': active, 'c-select_disabled': disabled }"
    class="c-select"
    @click="showPicker">
    <span
      v-if="selectedText"
      class="c-select-text">{{ selectedText }}</span>
    <span
      v-else
      class="c-select-placeholder">{{ _placeholder }}</span>
    <i class="c-select-icon"/>
  </div>
</template>
