<template>
  <div
    ref="formItem"
    :class="itemClass"
    class="c-form-item border-bottom-1px">
    <template v-if="!isBtnField">
      <slot name="label">
        <div
          v-show="fieldValue.label"
          class="c-form-label"><span>{{ fieldValue.label }}</span></div>
      </slot>
      <c-validator
        v-if="hasRules"
        ref="validator"
    ...
    <c-button
      v-else
      v-bind="fieldValue.props">{{ fieldValue.label }}</c-button>
  </div>
</template>
    ...
    computed: {
      ...
      itemClass() {
        ...
        return {
          ...
          'c-form-item_required': rules && rules.required,
          'c-form-item_btn': this.isBtnField,
          'c-form-item_validating': this.validating,
          'c-form-item_pending': this.pending,
          'c-form-item_valid': this.valid,
          'c-form-item_invalid': this.invalid
        }
      },
      modelVal() {
