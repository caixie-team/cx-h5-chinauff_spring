<template>
  <div
    ...
    >
      <div
        :style="fileStyle"
        class="c-upload-file-def">
        <i
          class="cubeic-wrong"
          @click.stop="removeFile"/>
        <div
          :class="fileStatusCls"
          class="c-upload-file-state">
          <i
            :class="statusCls"
            class="c-upload-file-status"/>
          <span class="c-upload-file-progress">{{ fileProgress }}</span>
        </div>
      </div>
    </slot>
  </div>
...
<script type="text/ecmascript-6">
  ...
  const COMPONENT_NAME = 'c-upload-file'
...
<style lang="stylus" rel="stylesheet/stylus">
  ...
  .c-upload-file-status
    position: relative
    z-index: 1
    font-size: $fontsize-large-xxxx
    display: none

  .c-upload-file-status.cubeic-right
    display: block
    color: $upload-file-success-color

  .c-upload-file-status.cubeic-warn
    display: block
    color: $upload-file-error-color

  .c-upload-file-status.cubeic-right, .c-upload-file-status.cubeic-warn
    &::after
      content: ""
      z-index: -1
      position: absolute
